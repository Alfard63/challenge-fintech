...
const deleteDataFromSQL = async (table, id) => {
  ...
  const response = await fetch("../../action/delete_data.php", {
    method: "POST",
    body: JSON.stringify({table, id, primaryKeyName}),
  });
  const data = await response.json();
  console.log(data);
  return data;
...
const clearForm = () => {
  ...
  document.querySelector("#editForm").setAttribute("action", "../../action/user_creation.php");
...
const viewUser = (id, edit) => {
  ...
  inputs.forEach((input) => {
    input.value = typeof(users[id][input.name]) === "string" ? (users[id][input.name]).toUpperCase() : users[id][input.name];
  ...
  ? (
        ...
        formBtn.style.display = "inline-block", 
        formModal.querySelector("h2").innerHTML = '<i class="bi bi-pencil-fill"></i> Modifier un utilisateur',
        document.querySelector("#editForm").setAttribute("action", `../../action/update_data.php?`)
        )
        : (selects[0].disabled = true, formBtn.style.display= "none", formModal.querySelector("h2").innerHTML = '<i class="bi bi-person-fill"></i> Visualiser l\'utilisateur');
};

//function to delete a ligne from the table
const deleteUser = async(userId) => {
  const response = await deleteDataFromSQL('users', userId);
  if (response === 'success') {
  alert(`L'utilisateur N°${userId} a été supprimé`, 'success', document.querySelector('main'), 'div');
  showUsersinTable();
  }
  else {
    alert(`Erreur lors de la suppression de l'utilisateur N°${userId}`, 'danger', document.querySelector('main'), 'div');
  }
};
...
const alert = (message, color, parent, elementType) => {
  ...
  wrapper.innerHTML =
    `<div class="alert alert-${color} alert-dismissible fw-bold fs-5" role="alert"> ${message} <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>`;
